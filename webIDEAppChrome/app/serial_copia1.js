...
var onReceiveCallback = function(info) {
	if ( info.connectionId && info.data) {
		...
		if (str.charAt(str.length-1) === '\n') {
			...
			stringReceived += str;
			console.log("recibiendo: " + str);
			if ((counter_buffer == -1) && ((str_before == '>') && (str == " "))){
				counter_buffer = 0;
				console.log("Habilitar");
			}
			if (counter_buffer != -1){
				console.log("prompt >>"+str_before  + str + "<<");
				if(((str == buffer_code[counter_buffer-1]) && (str != '\n')) || ((str_before == '>') && (str = " "))){
					console.log(buffer_code[counter_buffer]);
					writeSerial(buffer_code[counter_buffer]);
					counter_buffer ++;
				}
				else{
					console.log("no envia");
					console.log("str = " + str + ", buffer_code[counter_buffer] = " + buffer_code[counter_buffer] + ", str_before = " + str_before);
				}
			}
		}
		str_before = str;
